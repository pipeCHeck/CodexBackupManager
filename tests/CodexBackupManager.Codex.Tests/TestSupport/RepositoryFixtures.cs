...
/// <summary>
/// Repository에 커밋된 <c>tests/Fixtures/</c> 경로를 찾는다.
/// </summary>
/// <remarks>
/// 테스트 실행 디렉터리(<c>bin\Debug\net10.0</c>)에서 위로 올라가며
/// <c>CodexBackupManager.sln</c>이 있는 폴더를 Repository 루트로 본다.
/// </remarks>
public static class RepositoryFixtures
{
    ...
    /// <summary>
    /// 픽스처를 임시 폴더로 복사한다. 원본 픽스처를 건드리지 않기 위해 사용한다.
    /// </summary>
    ...
    private static string FindRepositoryRoot()
    {
        ...
        throw new InvalidOperationException(
            $"{SolutionFileName}을 찾을 수 없어 Repository 루트를 특정할 수 없습니다. " +
            $"탐색 시작: {AppContext.BaseDirectory}");
    }
}
